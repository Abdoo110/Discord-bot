...
async function checkSpam(message) {
  ...
  const now = Date.now();
  if (!guildSpam.has(message.author.id)) { guildSpam.set(message.author.id, { count: 1, firstMsgTime: now }); return false; }
  const data = guildSpam.get(message.author.id);
  if (now - data.firstMsgTime > windowMs) { data.count = 1; data.firstMsgTime = now; return false; }
  data.count++;
  if (data.count > maxMessages) {
    try {
      ...
      if (muteRole && message.member) {
        ...
        setTimeout(async () => { try { if (message.member) await message.member.roles.remove(muteRole, 'Auto-unmute: spam timeout expired'); } catch (_) {} }, muteDuration);
      }
    } catch (_) {}
    try {
...
async function checkRaid(member) {
  ...
  if (!raidCache.has(member.guild.id)) { raidCache.set(member.guild.id, { count: 1, windowStart: Date.now() }); return; }
  const data = raidCache.get(member.guild.id);
  const now = Date.now();
  if (now - data.windowStart > windowMs) { data.count = 1; data.windowStart = now; return; }
  data.count++;
    ...
    if (action === 'lockdown') {
      ...
      if (logChannelId) {
        ...
        if (logChannel) {
          ...
          logChannel.send({ embeds: [buildEmbed({ color: 'error', title: '🚨 ANTI-RAID ACTIVATED', description: `Raid detected! All channels locked down.\nJoins: ${data.count}\nWindow: ${windowMs / 1000}s`, timestamp: Date.now() })] });
        }
      }
    }
...
function getNukeCache(guildId) {
  if (!nukeCache.has(guildId)) nukeCache.set(guildId, { channelDeletes: 0, roleDeletes: 0, banKicks: 0, windowStart: Date.now(), locked: false });
...
async function checkAntiNuke(guild, type) {
  ...
  if (now - data.windowStart > windowMs) { data.channelDeletes = 0; data.roleDeletes = 0; data.banKicks = 0; data.windowStart = now; data.locked = false; }
  data[type]++;
  ...
  if (data[type] >= limits[type] && !data.locked) {
    ...
    for (const [, channel] of guild.channels.cache) {
      if (channel.isTextBased()) await channel.permissionOverwrites.edit(guild.roles.everyone, { SendMessages: false }).catch(() => {});
    }
    const logChannelId = cfg.channels.modLogs;
    if (logChannelId) {
      ...
      if (logChannel) {
        const { buildEmbed } = require('../utils/embed');
        logChannel.send({ embeds: [buildEmbed({ color: 'error', title: '🚨 ANTI-NUKE ACTIVATED', description: `Nuke attempt detected! All channels locked down.\nType: ${type}\nCount: ${data[type]}`, timestamp: Date.now() })] });
      }
    }
    return true;
  }
