...
async function processOrder(interaction) {
  ...
  const chId = cfg?.channels?.ordersChannel;
  if (!chId) return interaction.reply({ content: 'Order saved but no orders channel configured.', flags: MessageFlags.Ephemeral });
  const ordersChannel = interaction.guild.channels.cache.get(chId);
  if (!ordersChannel) return interaction.reply({ content: 'Order saved but orders channel not found.', flags: MessageFlags.Ephemeral });

  const orderEmbed = new EmbedBuilder()
    .setTitle(`New Order — ${interaction.user.tag}`)
    .setDescription(`**Shulker Type:** ${shulkerType}\n**Quantity:** ${quantity}\n**When Needed:** ${whenNeeded}\n**Total Price:** ${formatted}`)
    .setColor('#5865F2').setFooter({ text: `Order ID: ${order._id} | Pending` }).setTimestamp();
  ...
  order.orderChannelId = chId; order.orderMessageId = sent.id; await order.save();
...
async function confirmOrder(interaction) {
  ...
  try { const u = await interaction.client.users.fetch(order.userId); const dm = await u.createDM(); await dm.send({ embeds: [new EmbedBuilder().setTitle('Order Processing').setDescription(`Your order is getting processed, please wait.\n\n**Shulker Type:** ${order.shulkerType}\n**Quantity:** ${order.quantity}\n**Total Price:** ${order.formattedPrice}`).setColor('#FEE75C').setTimestamp()] }); } catch (_) {}

  try {
    const ch = interaction.guild.channels.cache.get(order.orderChannelId);
    if (ch) {
      const msg = await ch.messages.fetch(order.orderMessageId).catch(() => null);
      if (msg) {
        const emb = EmbedBuilder.from(msg.embeds[0]).setFooter({ text: `Order ID: ${order._id} | Processing` }).setColor('#FEE75C');
        const cBtn = new ButtonBuilder().setCustomId(`order_confirm_${order._id}`).setLabel(`Confirm Order — ${order.formattedPrice}`).setStyle(ButtonStyle.Success).setDisabled(true);
        const rBtn = new ButtonBuilder().setCustomId(`order_ready_${order._id}`).setLabel('User is Ready').setStyle(ButtonStyle.Primary);
        await msg.edit({ embeds: [emb], components: [new ActionRowBuilder().addComponents(cBtn, rBtn)] });
      }
    }
  } catch (_) {}
...
async function userReady(interaction) {
  ...
  const order = await Order.findById(orderId);
  if (!order) return interaction.reply({ content: 'Order not found.', flags: MessageFlags.Ephemeral });
  if (order.status === 'ready') return interaction.reply({ content: 'Already marked ready.', flags: MessageFlags.Ephemeral });
  order.status = 'ready'; await order.save();

  try {
    const ch = interaction.guild.channels.cache.get(order.orderChannelId);
    if (ch) { const msg = await ch.messages.fetch(order.orderMessageId).catch(() => null); if (msg) { const emb = EmbedBuilder.from(msg.embeds[0]).setFooter({ text: `Order ID: ${order._id} | Ready` }).setColor('#57F287'); await msg.edit({ embeds: [emb], components: [] }); } }
  } catch (_) {}
  ...
  try {
    ...
    collector.on('collect', async (msg) => {
      try {
        const oCh = interaction.guild.channels.cache.get(order.orderChannelId);
        if (oCh) await oCh.send({ embeds: [new EmbedBuilder().setTitle(`${user.username} is available`).setDescription(msg.content).setFooter({ text: `Order ID: ${order._id}` }).setColor('#5865F2').setTimestamp()] });
        await dm.send('Your availability has been sent!');
      } catch (_) {}
    });
    collector.on('end', c => { if (c.size === 0) dm.send('No reply received — contact staff directly.').catch(() => {}); });
  } catch (_) {}
