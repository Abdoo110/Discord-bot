process.on('unhandledRejection', (reason) => { console.error('[CRASH] Unhandled Rejection:', reason?.stack || reason); });
process.on('uncaughtException', (err) => { console.error('[CRASH] Uncaught Exception:', err.stack || err); });
...
async function connectDB() { try { await mongoose.connect(config.mongoUri); console.log(chalk.green('Connected to MongoDB')); } catch (err) { console.error(chalk.red('MongoDB connection error:'), err.message); process.exit(1); } }

client.once(Events.ClientReady, async () => {
  console.log(chalk.cyan(`Logged in as ${chalk.bold(client.user.tag)} — ${client.guilds.cache.size} guild(s)`));
  client.user.setPresence({ activities: [{ name: `${client.guilds.cache.size} server | join our support server`, type: 3 }], status: 'online' });
});
...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (interaction.customId.startsWith('giveaway_enter_')) {
      try {
        ...
        const updated = await Giveaway.findOneAndUpdate(
          ...
          { $push: { entrants: interaction.user.id } }, { new: true }
        );
        if (!updated) {
          const giveaway = await Giveaway.findOne({ guildId: interaction.guild.id, messageId });
          if (!giveaway || giveaway.ended) return interaction.reply({ content: 'This giveaway has ended or no longer exists.', flags: MessageFlags.Ephemeral });
        ...
        try {
          ...
          if (embed) {
            const newDesc = embed.description.replace(/\*\*Participants:\*\* \d+/, `**Participants:** ${updated.entrants.length}`);
            await msg.edit({ embeds: [require('./utils/embed').buildEmbed({ color: 'giveaway', title: embed.title, description: newDesc })] });
          }
        } catch (_) {}
        await interaction.reply({ content: 'You entered the giveaway! Good luck!', flags: MessageFlags.Ephemeral });
      } catch (err) { console.error('[GIVEAWAY-BUTTON] Error:', err.message); if (!interaction.replied) await interaction.reply({ content: 'Something went wrong. Try again.', flags: MessageFlags.Ephemeral }).catch(() => {}); }
    }
    return;
  }
  ...
  if (!command) return;
  try { await command.execute(interaction); } catch (err) { console.error(`/${interaction.commandName}:`, err.message); const { buildEmbed } = require('./utils/embed'); const reply = { embeds: [buildEmbed({ color: 'error', title: 'Error', description: 'Something went wrong.' })], flags: MessageFlags.Ephemeral }; if (interaction.replied || interaction.deferred) await interaction.followUp(reply); else await interaction.reply(reply); }
});
...
(async () => {
  try {
    await connectDB(); await deployCommands(); await client.login(config.token);
    ...
    setInterval(async () => {
      try {
        ...
        for (const gw of expired) {
          ...
          } catch (err) { console.error('[SAFETY] Error:', err.message); }
        }
      } catch (err) { console.error('[SAFETY] Check error:', err.message); }
    }, 120000);
  } catch (err) { console.error(chalk.red('Startup failed:'), err.stack || err.message); process.exit(1); }
})();

process.on('SIGINT', async () => { console.log('Shutting down...'); await mongoose.disconnect(); client.destroy(); process.exit(0); });
