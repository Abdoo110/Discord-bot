...
// In-memory tracking for active fast-click games
const activeGames = new Map(); // Map<messageId, { startedAt, winner, channelId }>
  ...
  async execute(interaction) {
    if (activeGames.has(interaction.channel.id)) {
      return interaction.reply({ embeds: [
        buildEmbed({ color: 'error', title: '⚠️ Already Active', description: 'There is already an active fast-click game in this channel.' })
      ], flags: MessageFlags.Ephemeral });
    }

    const embed = buildEmbed({
      color: 'fun',
      title: '⚡ FAST CLICK — First to React Wins!',
      description: `React with ⚡ as fast as you can!\n\nGame started by ${interaction.user.tag}`,
      footer: 'Hurry!',
      timestamp: Date.now(),
    });
    ...
    collector.on('end', async (collected) => {
      if (collected.size === 0) {
        activeGames.delete(interaction.channel.id);
        await interaction.channel.send({ embeds: [
          buildEmbed({ color: 'warn', title: '⏰ Time\'s Up!', description: 'No one clicked in time. Game over!' })
        ]});
      }
    });
  },
