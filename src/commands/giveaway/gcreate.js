...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const embed = buildEmbed({ color: 'giveaway', title: `🎉 ${prize}`, description: [`React with 🎉 to enter!`, `**Winners:** ${winners}`, `**Ends:** <t:${Math.floor(endsAt.getTime() / 1000)}:R>`, requirements ? `**Requirements:** ${requirements}` : '', '', `**Hosted by:** ${interaction.user.tag}`].join('\n'), footer: 'Ends at', timestamp: endsAt });
    await interaction.reply({ content: '🎉 **GIVEAWAY** 🎉', embeds: [embed] });
    const msg = await interaction.fetchReply();
    await msg.react('🎉');
    await Giveaway.create({ guildId: interaction.guild.id, messageId: msg.id, channelId: interaction.channel.id, hostId: interaction.user.id, prize, winners, durationMs, endsAt });
    ...
    if (cfg.channels.giveawayLogs) {
      ...
      if (logChannel) {
        logChannel.send({ embeds: [buildEmbed({ color: 'giveaway', title: '🎉 Giveaway Created', fields: [
          { name: 'Prize', value: prize, inline: true }, { name: 'Winners', value: `${winners}`, inline: true },
          { name: 'Duration', value: durStr, inline: true }, { name: 'Host', value: interaction.user.tag, inline: true },
        ], timestamp: Date.now() })] });
      }
    }
  },
